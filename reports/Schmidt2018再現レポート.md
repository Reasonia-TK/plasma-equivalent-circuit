...
- 論文は実際の SPICE netlist を公開していない。公開された整合点を再現するには、論文式に対する実効微分容量係数 `alpha_C=0.5862` が必要だった。この係数は `Im(Z_in)` の一点だけで同定し、残りの電圧・電流・電力は独立な検証量とした。

したがって、標準問題の数値再現という目的は達成した。ただし `alpha_C` は著者コードを入手して確認した値ではなく、公開結果から同定した経験パラメータである。物理的な行列シース微分容量は `alpha_C=0.5` となり、旧ngspice版にも規約差は見つからなかったため、`0.5862` を第一原理値とは解釈しない。
...
論文の `C_s∝V_s^-1/2` は、`Q/V` と微分容量 `dQ/dV` の解釈で係数2の違いが生じる。行列シースの電荷は `Q=sqrt(K V_s)` なので、論文記載式 `sqrt(K/V_s)` は `Q/V_s` に等しく、物理的な微分容量はその半分 `0.5 sqrt(K/V_s)` である。一方、論文には著者のnetlistがない。現在のngspice-46に加え、公式ソースのngspice-27/28も `C={式}` を `I=C(V)dV/dt` として扱うため、版差では係数を説明できない。
...
さらに `alpha_C=0.5` の負荷から解析的に逆算した整合容量は `C_m1=1544.1 pF`、`C_m2=176.9 pF` で、論文の丸め値に近かった。しかし `alpha_C=0.5` のままグローバルモデルと整合器を完全連成すると、`n_e=9.55e14 m^-3`、`P_pl=3.71 W`、`I_pl=0.460 A` となり、公開結果との差が残った。詳細は [容量規約監査](Capacitance規約監査.md) にまとめた。
...
1. `alpha_C` の由来を確定するには、著者の2018年版netlistまたは非線形容量コードが必要である。ngspice-27/28の規約は確認済みで、現時点では公開結果からの推定である。
...
5. `alpha_C` と整合器を固定した圧力 `0.5–10 Pa`、電源振幅 `50–200 V peak` の未使用条件予測を実施した。次は [予測マップ](PressureVoltage予測マップ.md) の識別力が高い条件から実測する。
